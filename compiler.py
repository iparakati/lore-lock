...
def compile_game(yaml_file):
    ...
    # Generate Game Code
    # We embed the data dictionary directly into the python file
    game_code = TEMPLATE % (repr(data))

    output_filename = os.path.splitext(yaml_file)[0] + "_game.py"
    ...
    test_code = TEST_TEMPLATE % (module_name, module_name, repr(test_commands), repr(win_condition))

    test_filename = "test_" + os.path.splitext(yaml_file)[0] + ".py"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python compiler.py <story.yaml>")
        sys.exit(1)
    compile_game(sys.argv[1])
